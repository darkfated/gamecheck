<template>
  <div
    class="game-quiz-container p-6 bg-gradient-to-br from-[var(--card-bg)] to-[var(--bg-tertiary)]/80 backdrop-blur-md rounded-2xl shadow-xl border border-[var(--border-color)] max-w-4xl mx-auto">
    <h2
      class="quiz-title text-3xl font-bold text-[var(--text-primary)] mb-6 bg-gradient-to-r from-indigo-500 to-purple-500 bg-clip-text text-transparent">
      Игровые тесты</h2>

    <!-- Test Selection UI -->
    <div v-if="!categorySelected && !quizStarted" class="category-selection">
      <h3 class="text-xl font-semibold text-[var(--text-secondary)] mb-4">Выберите категорию</h3>
      <div class="categories-grid grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        <div v-for="(category, index) in categories" :key="index" @click="selectCategory(category.id)"
          class="category-card p-6 bg-gradient-to-br from-indigo-500 to-purple-500 text-white rounded-xl shadow-md cursor-pointer hover:scale-105 transition-transform">
    ...
    <!-- Quiz Start UI -->
    <div v-else-if="categorySelected && !quizStarted" class="quiz-start text-center">
      <h3 class="text-2xl font-semibold text-[var(--text-primary)] mb-2">{{ selectedCategory.name }}</h3>
      <p class="text-[var(--text-secondary)] mb-4">{{ selectedCategory.description }}</p>
      <p class="text-[var(--text-secondary)]">Тест состоит из {{ selectedCategory.questions.length }} вопросов.</p>
      <div class="start-actions mt-6 flex justify-center gap-4">
        <button @click="startQuiz"
          class="start-button px-6 py-3 bg-gradient-to-r from-green-500 to-teal-500 text-white rounded-lg shadow hover:from-green-600 hover:to-teal-600 transition-all">Начать
          тест</button>
        <button @click="backToCategories"
          class="back-button px-6 py-3 bg-[var(--bg-secondary)] text-[var(--text-primary)] rounded-lg shadow hover:bg-[var(--bg-secondary-hover)] transition-all">←
    ...
    <div v-else-if="quizStarted && !quizFinished" class="quiz-question">
      <h3 class="text-xl font-semibold text-[var(--text-primary)] mb-4">Вопрос {{ currentQuestionIndex + 1 }} из {{
        currentQuestions.length }}</h3>
      <div class="question-text text-[var(--text-secondary)] mb-6">{{ currentQuestion.question }}</div>

      <div class="answers-container grid grid-cols-1 sm:grid-cols-2 gap-4">
        <button v-for="(answer, index) in currentQuestion.answers" :key="index" @click="selectAnswer(index)"
          :class="['answer-button px-4 py-3 rounded-lg shadow transition-all', selectedAnswer === index ? 'bg-indigo-500 text-white' : 'bg-[var(--bg-secondary)] text-[var(--text-primary)] hover:bg-[var(--bg-secondary-hover)]']">
          {{ answer }}
        </button>
      </div>

      <div class="navigation-buttons mt-6 flex justify-end">
        <button @click="nextQuestion" :disabled="selectedAnswer === null"
          class="next-button px-6 py-3 bg-gradient-to-r from-blue-500 to-indigo-500 text-white rounded-lg shadow hover:from-blue-600 hover:to-indigo-600 transition-all disabled:opacity-50 disabled:cursor-not-allowed">Далее</button>
      </div>
    </div>

    <!-- Quiz Results UI -->
    <div v-else-if="quizFinished" class="quiz-results text-center">
      <h3 class="text-2xl font-semibold text-[var(--text-primary)] mb-4">Результаты теста "{{ selectedCategory.name }}"
      </h3>
      <p class="text-[var(--text-secondary)] mb-4">Вы правильно ответили на {{ correctAnswers }} из {{
        currentQuestions.length }} вопросов!</p>
      <div class="result-score text-4xl font-bold text-[var(--accent-primary)] mb-6">
        {{ Math.round((correctAnswers / currentQuestions.length) * 100) }}%
      </div>
      <div class="results-actions flex justify-center gap-4">
        <button @click="restartQuiz"
          class="restart-button px-6 py-3 bg-gradient-to-r from-purple-500 to-pink-500 text-white rounded-lg shadow hover:from-purple-600 hover:to-pink-600 transition-all">Пройти
          заново</button>
        <button @click="backToCategories"
          class="back-button px-6 py-3 bg-[var(--bg-secondary)] text-[var(--text-primary)] rounded-lg shadow hover:bg-[var(--bg-secondary-hover)] transition-all">←
          Другие тесты</button>
